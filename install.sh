...
if not obj:
    ...
    for i, inb in enumerate(obj, 1):
        ...
        port = inb.get("port", "?")
        print(f"{i}) id={inb.get('id')}  [{protocol}]  port={port}  remark=\"{remark}\"")
PYEOF
        echo ""
        read -rp "Enter the numbers of inbounds to use, comma-separated (e.g. 1,3): " INB_CHOICES
...
import json, sys
data = json.loads(sys.argv[1])
for i, inb in enumerate(data.get("obj", []), 1):
    remark = inb.get("remark") or f"inbound-{inb.get('id')}"
    print(f"{i}) id={inb.get('id')}  [{inb.get('protocol')}]  port={inb.get('port')}  remark=\"{remark}\"")
PYEOF
            echo ""
            read -rp "Enter numbers to include (comma-separated): " PICKS
            echo ""
            read -rp "Enable Vision flow (xtls-rprx-vision) for these inbounds? (y/n): " ISV
            FLOW=""
