...
async def buy_confirm(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    if balance >= total:
        ...
        await cb.answer()
        try:
            await _deliver_service(cb.bot, tg_id, gb, total, "new", service_name)
        except Exception as e:
            logger.exception("Purchase deliver_service failed: %s", e)
...
async def buy_receipt(message: Message, state: FSMContext) -> None:
    ...
    pid = await db.create_pending_payment(
        ...
        json.dumps({"gb": gb, "total": total, "service_name": service_name}),
    )
    await state.clear()
