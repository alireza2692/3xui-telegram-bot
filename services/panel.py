...
class PanelClient:
    ...
    def _default_flow(self) -> str:
        """
        The first non-empty flow among configured inbounds. Empty string if
        none of them are Reality (rather than assuming Vision by default,
        which would break plain/non-Reality inbounds).
        """
    ...
    async def update_client(
        ...
        """
        Update an existing client in-place.

        IMPORTANT: `client_id` is the actual connection UUID (the `id` field
        the panel uses in the VLESS link), which is a *different* value from
        `sub_id` (used for the subscription URL). Passing a new `client_id`
        here rotates the connection UUID via an update call instead of a
        delete+add cycle. This matters because usedTraffic is tracked by
        email in the panel's client_traffics table -- delete removes that
        row, add recreates it at zero. update never touches it, so traffic
        is preserved.

        `flow` should normally be the client's EXISTING flow (fetched via
        get_client) so renew/change-link never silently flips a non-Reality
        client to Vision or vice versa. Falls back to the configured default
        only if the caller doesn't know the current value.
        """
        if flow is None:
            flow = self._default_flow()
        payload = {
    ...
    async def renew_client(self, email: str, add_gb: int) -> None:
        ...
        # Preserve the client's existing flow -- do not silently change it on renew
        await self.update_client(
            email, c["subId"], int(new_total), new_expire,
            client_id=c["id"], flow=c.get("flow", ""),
        )

    async def change_link(self, email: str) -> str:
        """
        Rotate the client's connection UUID and subId WITHOUT resetting
        usedTraffic. Previously this did delete_client + add_client, which
        wiped the client_traffics row (keyed by email) and reset usage to 0.
        Now it does a single update call: same email, new id (connection
        UUID) and new subId, same totalGB/expiryTime -- so the panel never
        drops the traffic-tracking row.
        """
        c = await self.get_client(email)
        if not c:
            raise PanelError(f"Client not found: {email}")
        ...
        # Preserve the client's existing flow here too
        await self.update_client(
            email,
            new_sub,
            c["totalGB"],
