...
class PanelClient:
    ...
    async def add_client(self, email: str, sub_id: str, traffic_bytes: int, expire_ms: int,
        ...
        if inbound_ids is None:
            inbound_ids = [i_id for i_id, _remark, _flow in cfg.inbounds] if cfg.inbounds else [cfg.inbound_id]
        if flow is None:
            # Use the first non-empty flow among configured inbounds
            # (client-level flow is a single value shared across all attached inbounds)
            flow = next((f for _id, _remark, f in cfg.inbounds if f), "") if cfg.inbounds else "xtls-rprx-vision"
        payload = {
            "client": {
                "email": email,
                "subId": sub_id,
    ...
    async def renew_client(self, email: str, add_gb: int) -> None:
        ...
        await self.update_client(email, c["subId"], int(new_total), new_expire, client_id=c["id"])
